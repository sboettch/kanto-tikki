...
function calcCommute(stId, destId){
  ...
  if (!a || !b) return { median: 25, p90: 29, direct: true, transfers: 0, verified: false, walkOnly: false };
...
function applyChrome(){
  ...
  $('#goalchip').textContent = `goal ${activeGoalMin()}′`;
...
function filteredHomes(){
  ...
  let hs = allHomes.filter(l => {
    ...
    if (state.fQuick==='goal' && Math.abs(calcCommute(l.st, state.dest).median - goalMin) > 5) return false;
...
function homesView(){
  ...
  const nGoal = allHomes.filter(l => Math.abs(calcCommute(l.st, state.dest).median - goalMin) <= 5).length;
  ...
  <div class="quick-chips" role="group" aria-label="Quick filters">
    ...
    <button class="quick-chip ${state.fQuick==='goal'?'on':''}" data-qfilter="goal">
      ⏱️ ${esc(LANG.cur==='ja'?'目標圏 ±5分':'Within ±5′ Goal')} <span class="q-count">${nGoal}</span>
    </button>
...
function openCase(id){
  const l = HOMES.find(h => h.id===id); if (!l) return;
  const st = STATIONS[S_IDX[l.st]];
  const eff = (l.deposit_mo!=null||l.key_mo!=null||l.deposit_yen!=null||l.key_yen!=null) ? effCost(l) : null;
  const commRows = DESTS.map(d => {
    const c = commute(l.st, d.id);
...
/* ————— tikki radio ————— */
function radioDeck(){
  const live = HOMES.filter(l => l.tier==='LIVE' && l.rent);
...
function radioView(){
  ...
  const l = r.deck[r.i];
  const st = STATIONS[S_IDX[l.st]];
  const c = commute(l.st, state.dest);
  if (l.approx) c.approxMark = '≈';
  const a = areaFor(l.st);
  ...
  <div class="deckwrap"><div class="deckcard">
    ...
    <div class="commline" style="padding:12px 16px 14px">
      ...
      ${noiseSummary(l.st)?`<span class="badge b-transfer" title="${esc(tx(noiseSummary(l.st)))}">⚠ ${esc(t('noise_chip'))}</span>`:''}
      <span class="badge b-goal">${(c.median-GOAL_MIN)>0?'+':''}${(c.median-GOAL_MIN).toFixed(1)}′</span>
    </div>
  ...
  <p class="provrow" style="text-align:center;margin-top:8px">${esc(LANG.cur==='ja'?'← パス · → 好き · スペースで調書':'← pass · → love · space = case file')}</p></div>`;
}
  ...
  if (state.view === 'homes') m.innerHTML = homesView();
    ...
    if (sc){  // center the map on the station closest to the goal
      ...
      STATIONS.forEach((s,i) => {
        const c = commute(s.id, state.dest);
        if (!c.walkOnly && Math.abs(c.median-GOAL_MIN) < bv){ bv = Math.abs(c.median-GOAL_MIN); bi = i; }
      });
